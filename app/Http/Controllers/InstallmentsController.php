...
class InstallmentsController extends Controller
{
    ...
    public function payByAlipay(Installment $installment){
        ...
        if($installment->sataus === Installment::STATUS_FINISHED){
            throw new InvalidRequestException('该订单分期已经结清');
        }
    ...
    public function alipayNotify(){
        ...
        //根据还款计划编号查询出对应的还款计划,原则上不会找不到,这里的判断只是增强代码健壮性
        if(!$item=$installment->items()->where('sequence',$sequence)->first()){
            return 'fail';
        }
        //如果这个还款计划的试支付状态是已支付,则告知支付宝此订单已完成,并不在执行后续逻辑
        if($item->paid_at){
            return app('alipay')->success();
        }
        //使用使用,保证数据一致性
