...
<div class="row">
    <div class="col-lg-10 offset-lg-1">
                ...
                <div class="row">
                    ...
                    <div class="col-7">
                        ...
                        <div class="crowdfunding-info">
                            ...
                            <div class="progress">
                                <div class="progress-bar progress-bar-success progress-bar-striped" role="progressbar" aria-valuenow="{{ $product->crowdfunding->percent }}"
                                    aria-valuemin="0" aria-valuemax="100" style="min-width: 1em; width: {{ min($product->crowdfunding->percent, 100) }}%">
                                </div>
                            </div>
                        ...
                        @else
                        <div class="price"><label for="">价格</label><em>￥</em><span>{{$product->price}}</span></div>
        ...
        <!-- 众筹商品模块结束 -->
                            ...
                            <div class="cart_amount"><label>数量</label><input type="text" class="form-control form-control-sm" value="1"><span>件</span>
                                <span class="stock"></span>
                            </div>
                            <div class="buttons">
                                @if($favored)
                                <button class="btn btn-danger btn-disfavor">取消收藏</button> @else
                                <button class="btn btn-success btn-favor">❤ 收藏</button> @endif
                                <!-- 众筹商品下单按钮开始 -->
                                @if($product->type === \App\Models\Product::TYPE_CROWDFUNDING) @if(Auth::check()) @if($product->crowdfunding->status ===
                                \App\Models\CrowdfundingProduct::STATUS_FUNDING)
                                <button class="btn btn-primary btn-crowdfunding">参与众筹</button> @else
                                <button class="btn btn-primary disabled">
                  {{ \App\Models\CrowdfundingProduct::$statusMap[$product->crowdfunding->status] }}
                </button> @endif @else
                                <a class="btn btn-primary" href="{{ route('login') }}">请先登录</a> @endif @else
                                <button class="btn btn-primary btn-add-to-cart">加入购物车</button> @endif
                                <!-- 众筹商品下单按钮结束 -->
                            </div>
                        </div>
                    </div>
                    <div class="product-detail">
                        <ul class="nav-tabs nav" role='tablist'>
                            <li class="nav-itme">
                                <a href="#product-detail-tab" class="nav-link active" ria-controls="product-detail-tab" role="tab" data-toggle="tab" aria-selected="true">
                                    商品详情
                                </a>
                            </li>
                            <li class="nav-itme">
...
@section('scriptsAfterJs')
    ...
    $('.btn-favor').click(function(){
        // 发起一个post ajsx请求,请求URl通过后端的route()函数生成
        axios.post('{{ route('products.favor', ['product' => $product->id]) }}')
            .then(function (){ //请求成功会执行这个函数
                swal('操作成功','','success')
                .then(function (){
                    //
                    location.reload();
                });
            },function (error){ //请求失败会执行这个函数
                //如果返回的是401 代表灭有登陆
    ...
    $('.btn-crowdfunding').click(function(){
        ...
        //吧用户的收货地址以json的形式放入页面,赋值给addresses变量
